...
- 本地数据：真实运行数据保存在 Git 忽略的 `data/local/`；仓库只保留 `data/*.example.json` 示例结构。后续可按 `docs/project_database_design.md` 迁移到 SQLite。
...
- AI 回填：粘贴 AI 输出后自动预判分类，并允许手动勾选为项目进展、决策记录、新任务、AI 建议和风险提醒。
...
```text
~/Desktop/SoloOS/data/local/
├── projects.json
├── ai_handoff_logs.json
└── project_updates.json
```

`data/local/` 已加入 `.gitignore`，因此日常使用 SoloOS 不会让 Git 工作区因为项目数据变化而变脏。
